...
export default function Prog() {
  ...
  const [selected, setSelected] = useState<ProgRow | null>(null);
  ...
  function normalizeArray(payload: unknown): ProgRow[] {
    ...
    if (isRecord(payload) && Array.isArray(payload.$values)) {
      return payload.$values as ProgRow[];
    }
    ...
    if (Array.isArray(possible)) return possible as ProgRow[];

    const deep = findArrayDeep(payload, 0);
    if (deep) return deep as ProgRow[];

    if (isRecord(payload)) return [payload as ProgRow];
    return [];
  }
  ...
  function onRowClick(row: ProgRow) {
    setSelected(row);
    setMode("view");
  }
  ...
  async function onUpdate() {
    if (!selected) {
      showToast("error", "Selecciona un prog para editar.");
      return;
    }
    ...
    if (selectedId == null) {
      showToast("error", "No se pudo resolver el idProg.");
      return;
    }
    ...
    try {
      ...
      showToast("success", "Prog actualizado correctamente");
      setMode("view");
      setSelected(null);
        ...
        <div className={styles.headerTop}>
          ...
          <div className={styles.headerActions}>
            ...
            <button
              ...
              type="button"
            >
              {mode === "create" ? "Creando..." : "+ Nuevo prog"}
            </button>
          </div>
        </div>
      </div>
          ...
          <div className={styles.panelBody}>
            {mode === "create" ? (
              ...
              <div className={styles.helper}>
                Selecciona un prog de la tabla para ver detalles.
              </div>
            ) : mode === "edit" ? (
              <form
                className={styles.form}
...
function getIdProg(r: ProgRow | null): number | null {
  if (!r) return null;
  const v = r.idProg ?? r.IdProg;
  const n = Number(v);
  return Number.isFinite(n) ? n : null;
}

function getCode(r: ProgRow | null): number | null {
  if (!r) return null;
...
function getDescription(r: ProgRow | null): string | null {
  if (!r) return null;
  const s = asTrim(r.description ?? r.Description ?? "");
  return s ? s : null;
}

function getActive(r: ProgRow | null): boolean | null {
  if (!r) return null;
