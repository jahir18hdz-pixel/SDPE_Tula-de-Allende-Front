...
export default function Community() {
  ...
  async function loadAll(keepSelectedCode?: number | null) {
    ...
    try {
      ...
      if (keepSelectedCode != null) {
        ...
        const found =
          normalized.find((r) => getIdCommunity(r) === selectedId) ?? null;
        setSelected(found);
  ...
  function normalizeArray(payload: unknown): CommunityRow[] {
    ...
    if (isRecord(payload) && Array.isArray(payload.$values)) {
      return payload.$values as CommunityRow[];
    }
    ...
    if (deep) return deep as CommunityRow[];

    if (isRecord(payload)) return [payload as CommunityRow];
    return [];
  }
        ...
        <div className={styles.headerTop}>
          ...
          <div className={styles.headerActions}>
            ...
            <button
              ...
              type="button"
            >
              {mode === "create" ? "Creando..." : "+ Nueva comunidad"}
            </button>
          </div>
        </div>
      </div>
          ...
          <div className={styles.panelBody}>
            {mode === "create" ? (
              ...
              <div className={styles.helper}>
                Selecciona una comunidad de la tabla para ver detalles.
              </div>
            ) : mode === "edit" ? (
              <form
                className={styles.form}
